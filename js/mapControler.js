            ...
            else
            {
                ...
                for(let key in Object.keys(targets))
                {
                    ...
                    else if(parseInt(roll) >= parseInt(ac))
                    {
                        display += `(Success Hit) ${targets[key].title.split(":")[0]} (${ac}), `;
                        fail = false; 
                        if(wholeChar[targets[key].title.split(":")[0]]){let targe = targets[key].title.split(":")[0]; if(wholeChar[targe]["rage"] && !description.includes("{noRage")){let past = damage.slice("**" + 2); past = past.slice(0, "**"); let hp = parseInt(hp)/2; damage = damage.replace(past, `**${hp}** (1/2 Rage)`);}}
                        handleChangeHp(damage.split("**")[1], wholeDb[targets[key].title.split(":")[0]], "-");
